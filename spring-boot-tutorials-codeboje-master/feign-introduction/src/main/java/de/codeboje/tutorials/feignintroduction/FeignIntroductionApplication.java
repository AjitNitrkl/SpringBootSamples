...
public class FeignIntroductionApplication implements ApplicationRunner {
	...
	@Autowired
	FeignKanbanClient feignKanbanClient;
	...
	public void run(ApplicationArguments args) throws Exception {
		String username = "testmy1234";
		String password = "mytestpaswd";
		...
		board = feignKanbanClient.createBoard(token, board);
	...
	private String loginUser(String username, String password) {
		...
		ResponseEntity<Void> response = feignKanbanClient.loginUser();
				//kanbanClient.loginUser("Basic " + buildBasicAuth(username, password));
